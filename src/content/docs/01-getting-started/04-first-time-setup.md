...
安装完 Git 后，你需要做的第一件事就是设置你的用户名和电子邮件地址。这一点非常重要，因为每次 Git 提交都会使用这些信息，并且它们会写入到你每一次的提交中，不可更改。
...
:::caution[重要提示]
这里的邮箱地址通常应该与你在 GitHub/GitLab 等代码托管平台上注册的邮箱保持一致，否则你的代码提交可能无法正确关联到你的账户（即没有“绿格子”贡献统计）。
:::
