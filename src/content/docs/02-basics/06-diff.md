...
当你修改了文件但还没有执行 `git add` 时，这些改动位于**工作区**。使用不带参数的 `git diff` 命令可以查看**工作区**与**暂存区**之间的差异。

:::tip[关键区别]
当没有**已暂存的变更**（即索引 index 与 HEAD 一致）时，`git diff` 的效果等价于比较工作区与 HEAD。
一旦暂存了内容（索引与 HEAD 不同），`git diff` 就变为比较**工作区 vs 索引**，只显示尚未暂存的修改。
:::
