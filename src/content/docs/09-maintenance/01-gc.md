...
通常情况下，你不需要手动运行它。当你运行 `git push` 或 `git merge` 等命令时，如果 Git 觉得垃圾太多，它会自动运行 `git gc --auto`。
...
git gc --aggressive
```
`--aggressive` 参数会让 Git 花更多时间来寻找最优的压缩差异（delta compression），通常能进一步减小仓库体积。建议在清理大仓库时运行一次。
...
默认情况下，`git gc` 只会清理两周前（默认配置）的悬空对象，以防你误删了还有用的数据。
