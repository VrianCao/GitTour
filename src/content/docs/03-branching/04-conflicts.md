...
:::note[其他冲突类型]
以上是最常见的冲突场景，但并非完整列表。其他可能引发冲突的情况包括：两个分支都新增了同名文件（add/add 冲突）、两个分支分别重命名同一文件为不同名称（rename/rename 冲突）、二进制文件冲突、子模块冲突等。
:::
...
它会依次打开冲突文件，提供三路合并（3-way merge）视图，通常包含 ours（当前分支）、theirs（被合并分支）、base（共同祖先）三个版本。具体的左右布局因所使用的工具而异。
