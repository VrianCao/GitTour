...
在 Git 中，分支操作极其轻量。正如我们在上一章所学，创建分支本质上只是**新建了一个指向特定提交的指针**（只有 41 字节大小的文件）。
...
**后果**：
如果你在这个状态下修改代码并 `commit`，这些新的提交将**不属于任何分支**。一旦你切换回其他分支，刚才写的那些提交就会因为没有分支指针引用而变成“孤魂野鬼”，最终被 Git 的垃圾回收机制清除。
