...
没错，它不是文件的物理副本，也不是什么复杂的容器。分支是一个 **ref**（引用），指向某个提交对象。在默认配置下，它通常体现为 `.git/refs/heads/<分支名>` 文件里的一行哈希值（Git 也可能将其打包到 `packed-refs` 文件中以优化性能）。
...
在底层文件系统中，创建分支的操作几乎是瞬间完成的，因为这只需要在 `refs/heads/` 目录中添加一条新的引用记录。
...
*   **main 是什么？**
    *   近年来，由于社区倡导更包容的术语，许多平台（如 GitHub 新建仓库）默认使用 `main` 作为主分支名。Git 本身的默认名称可通过 `init.defaultBranch` 配置项设置，不同环境（操作系统发行版、安装方式）可能仍默认 `master`，以 `git branch` 或初始化时的输出为准。`main` 和 `master` 在功能上没有任何区别。
...
| 特性 | SVN / 老式 VCS | Git |
| :--- | :--- | :--- |
| **创建分支方式** | 复制整个项目的目录到新位置 | 添加一条指向提交的引用记录 |
| **创建速度** | 慢（取决于项目文件大小） | **瞬间** |
